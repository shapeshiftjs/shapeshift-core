...
test('isString() is true for string', () => {
  expect(v.isString('hello')).toEqual(true);
});

test('isString() is false for number', () => {
  expect(v.isString(123)).toEqual(false);
});

test('isString() is false for null', () => {
  expect(v.isString(null)).toEqual(false);
});

test('isString() is false for undefined', () => {
  expect(v.isString(undefined)).toEqual(false);
});

test('isString() is false for object', () => {
  expect(v.isString({})).toEqual(false);
});

test('isString() is false for array', () => {
  expect(v.isString([])).toEqual(false);
});
